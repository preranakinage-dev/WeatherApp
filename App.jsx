...
const App = () => {
  ...
  const getWeather = async () => {
    ...
    toast.info(`Fetching weather data for ${capitalFirstLetter(cityName)}`);
  ...
  const formatBackground = () => {
    if (!weather) return 'from-cyan-600 to to-blue-700';
    const thershold = units === "metric" ? 20 : 60;
    if (weather.temp <= thershold) return "from-cyan-600 to-blue-700";
  ...
  return (
    ...
    shadow-xl shadow-gray-400 ${formatBackground()}`}>
      ...
      { weather && (
        ...
        <Forecast title="daily forecaste" data={weather.daily}/>
        </>
      )}
